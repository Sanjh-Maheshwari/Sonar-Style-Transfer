...
def save_and_maybe_display(optimizing_img, dump_path, config, img_id, num_of_iterations, should_display=False, image_name=""):
    ...
    if img_id == num_of_iterations-1 or (saving_freq > 0 and img_id % saving_freq == 0):
        ...
        cv.imwrite(os.path.join(dump_path, image_name + img_format[1]), dump_img[:, :, ::-1])
